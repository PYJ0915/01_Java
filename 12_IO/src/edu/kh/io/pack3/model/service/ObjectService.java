...
public class ObjectService {
	 ...
	 * 
	 * - Java 객체를 입/출력 할 때 사용하는 바이트 "보조" 스트림 (단독 사용 불가!)
	 * 
	 * *** 직렬화(Serializable) ***
	 *  - 객체를 직렬 형태로 변환
	 *   -> 객체의 상태(필드...) 를 바이트 스트림으로 변환하는 과정
	 *   -> 이를 통해 객체는 메모리에서만 존재하는 것이 아니라, 파일로 저장되거나 네트워크를 통해 전송될 수 있음
	 *   
	...
	public void objectOutput() {
		...
		try {
			...
			// NotserializableException -> 직렬화 되지 않음!
			...
			System.out.println("회원 데이터 출력완료!");
	...
	public void objectInput() {
		...
		try {
			...
			// 스트림을 이용하여 파일에 작성된 직렬화된 Member 객체를 읽어와서
			// 역직렬화를 수행하여 정상적인 Member 객체로 변경!
			// Object readObject() : 직렬화된 객체를 읽어와 역직렬화 + Member 객체로 변경(다운캐스팅)
