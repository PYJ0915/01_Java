...
public class FileService {
	...
	/**
	 * File 클래스로 객체 생성 + 폴더가 존재하지 않으면 원하는 경로에 폴더 생성
	 */
	public void method1() {
		
		// C:/io_test/20251014 폴더를 지정
		// 제일 앞에 "/" 시작의 의미: 현재 실행 중인 프로젝트의 루트 디렉토리 즉, 드라이브를 의미
		// 루트 디렉토리 : 최상위 폴더, 절대경로의 기준점
		
		// 명시적으로 맨 앞에 C: , D: 작성시 해당 드라이브에 폴더 생성 (== 절대 경로 작성법)
		
		/* io_test/20251014 작성 시 해당 프로젝트가 현재 위치! (== 상대 경로 작성법)
		...
		File directory = new File("/io_test/20251014"); // Java와 해당 디렉토리 연결
	...
	public void method2() {
		...
		// 문자열에서 "\" 는 이스케이프 문자로 사용됨 -> \ 자체를 문자로 사용하고 싶을 땐 2개를 사용 "\\"
	...
	public void method3() {
		...
		for(File f : files) {
			...
			// 마지막으로 수정한 날짜
			long lastModified = f.lastModified(); // ex. 616516811315631561
			
			// java.text.SimpleDateFormat : 간단히 날짜 형식을 지정할 수 있는 객체
			SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd a h:mm");
													  // 2025-10-14 오후 12:43
			
			// String SimpleDateFormat.format(long) : 매개변수 long 값을 지정된 패턴 형식으로 변환하여 문자열 형태로 반환하는 메서드
			...
			if(f.isDirectory()) { // 폴더 크기를 구하는 메서드는 따로 존재 X -> 크기를 구하려면 모든 파일 순회 후 각 파일의 크기 합산하여 사용!
