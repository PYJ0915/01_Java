...
public class Array2Example {
	...
	/* 2차원 배열
	 - 자료형이 같은 1차원 배열을 묶음으로 다루는 것 (배열의 배열)
	 -> 행,열 개념이 추가
	 */
	...
	public void ex1() {
		...
		// 2차원 배열의 선언
		int[][] arr; //== int arr[][]; == int[] arr[]; (대괄호 개수가 중요)
		// -> int 2차원 배열을 참조하는 참조변수 arr 선언
		
		// cf) 참조 변수 == 참조형 == 레퍼런스 변수 == 레퍼런스
		...
		*/ 
		...
		// Arrays.toString(): 참조하고 있는 1차원 배열의 값을 문자열로 반환
		 System.out.println(Arrays.toString(arr));
		 
		 //Arrays.deepToString(): 참조하고 있는 배열의 실제 데이터가 나오는 부분까지 깊게 파고들어서 모든 값을 문자열로 반환
