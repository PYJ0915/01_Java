...
import edu.kh.poly.ex2.model.service.PYJCalculator;
...
public class AbstractRun {
	...
	public static void main(String[] args) {
		...
		// 인터페이스 == 미완성 설계도 == 객체 생성 불가능 but 참조 변수로는 사용 가능
		
		// 코드의 큰 수정 없이 자식 객체 생성 코드만 바꾸면 새로운 클래스 코드 수행 가능
	
		// 인터페이스 특징
		// 1) 인터페이스를 부모 참조변수로 사용하면 다형성 중 업캐스팅이 적용되어서
		//    상속 받은 모든 클래스를 자식 객체로 참조할 수 있다. -> 중요한 메인코드의 수정 최소화 가능
		// 2) 자식 클래스의 공통된 메서드를 강제화 하기 때문에 모든 자식 클래스가 동일한 형태를 띄게 된다.
		// 	  -> 이를 이용하여 공동작업(팀 프로젝트)에 필요한 개발환경 조성에 용이하다!
