...
public class ExceptionService {
	...
	public void ex1() {
		...
		System.out.print("정수 1 입력: "); // java.util.InputMismatchException
		...
		try {
			...
			// e를 사용하여 예외 추적 코드를 작성할 수도 있음 -> 이 예외를 어떻게 해결할지 작성하는 구문
			System.out.println("0으로 나눌 수 없음");
			e.printStackTrace();
			// 발생한 예외에 대한 메서드와 위치에 대한 모든 내용을 출력하는 메서드
			// 예외 발생 지점 추적 메서드
			
		}
		
		// 발생하는 예외 중 일부 예외들은 try-catch를 사용하지 않아도 if-else문으로 예외 상황 방지 가능
	...
	public void ex2() {
	   ...
	   /* } catch(RuntimeException e) {
			...
			-> 여기서 다 잡혀버려서 아래 코드 도달 불가능 
			  ** 상속관계 유의해서 catch 매개 변수 설정! **
		...
		*/
		...
		} catch (RuntimeException e) { // NullPointerException을 써도 되지만 위에서 세부적인 예외를 잡았기 때문에 상위타입인 RuntimeException 사용
			
			System.out.println("잡았다!!");
			
		} catch (Exception e) {
			// 다형성 - 업캐스팅: 부모타입 참조 변수로 자식객체 참조
			// -> 상위 타입의 예외 클래스를 catch문에 작성하면 업캐스팅에 의해 모든 후손 예외 클래스를 잡아서 한 번에 처리 가능!
			
			System.out.println("모르는 예외는 내가 처리했으니 걱정 말라구");
			e.printStackTrace();
	...
	public void ex3() {
		...
		try {
			...
			e.printStackTrace(); // 상세히 예외를 추적해주는 메서드
			
		} finally {
			
			System.out.println("무조건 수행됨");
			sc.close(); // Scanner 통로 닫기 -> 메모리 누수 방지용
			
		}
		
		// try-with-resources: finally에서 일일이 close()를 호출하지 않아도 자동으로 리소스를 닫아주는 방식
	...
	public void methodC() throws FileNotFoundException {
		// throws: 호출한 메서드에게 예외를 던짐 -> 호출한 메서드에게 예외 처리 위임
	...
	public void ex5() throws UserException{
		/* 어떠한 상황이 발생함..
		-> 이 상황에 맞추어 특별한 예외를 발생시키고 싶은데 자바가 만들어둔 예외 클래스 중 마땅한 게 없을 때
		   사용자가 원하는 모양새의 예외 클래스를 만들어 쓸 수 있다 -> 사용자 정의 예외 클래스
		   UserException 사용하여 강제 예외 발생 시키기 */
